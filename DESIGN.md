...
`cwd` 和 `branch` 是必填字段。Hando 在 `save` 时自动采集当前工作目录和分支，确保接手的 Agent 知道应该进入哪个工作区、从哪个分支继续。`git_remote` 有则写入；本地仓库没有 remote 时可以为空。
...
如果当前目录不是 Git 仓库，仍允许保存交接包，但 `branch` 写为 `not_a_git_repository`，并在“当前代码状态”中写明 Git 不可用。
...
- `summary`：创建新任务时必填，更新已有任务时可选。它是 Agent 写出的当前交接正文，用于说明任务背景、当前进度、后续工作和注意事项。
- `id`：可选，指定任务 id。
- `project`：可选，项目名。
- `tags`：可选，标签列表。
...
推荐的 `summary` 至少包含：任务背景、任务目标、当前实现、当前进度、后续工作、验证情况、风险和给下一个 Agent 的明确指令。
...
调用 `save` 前，Agent 必须先整理交接说明。Hando 负责写文件，Agent 负责写内容。
...
- 在非 Git 目录调用 `save`。
- 验证仍可保存 `task.md`。
- 验证 frontmatter 包含 `cwd` 和 `branch: not_a_git_repository`。
